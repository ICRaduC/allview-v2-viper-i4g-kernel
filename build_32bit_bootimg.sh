#!/bin/bash
# Build 32-bit Boot Image Script
# Usage: ./build_32bit_bootimg.sh
set -e

KERNEL_DIR="$(cd "$(dirname "$0")" && pwd)"
KERNEL_IMAGE="$KERNEL_DIR/output_32bit/zImage"
RAMDISK_FIXED="/tmp/ramdisk_32bit.img"
OUTPUT_BOOT="$KERNEL_DIR/boot_32bit.img"

echo "=== Building 32-bit Boot Image ==="

# Check if kernel is built
if [ ! -f "$KERNEL_IMAGE" ]; then
    echo "ERROR: Kernel not built. Run './build_32bit.sh' first."
    exit 1
fi
